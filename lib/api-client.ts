...
const getBaseUrl = () => {
  ...
  // Server-side: use absolute URL or relative for same-origin
  // Vercel provides VERCEL_URL, but we should use NEXT_PUBLIC_APP_URL for reliability
  ...
  static async fetchRaw<T>(
    ...
    if (data?.code === 'STEP_UP_REQUIRED') {
      const resumeAttempt =
        ...
        ((options!.headers as Record<string, string>)['x-step-up-resume'] ===
          'true' ||
          (options!.headers as Record<string, string>)['X-Step-Up-Resume'] ===
            'true');
      if (typeof window !== 'undefined') {
