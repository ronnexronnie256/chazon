...
export async function updateSession(request: NextRequest) {
  ...
  if ((isOnDashboard || isOnSettings || isOnBookings || isOnAdmin) && !user) {
  ...
  if (isOnAuth && user && !isOnResetPassword) {
    // Don't redirect on callback page - it's handled there with role-based logic
    const isOnCallback = request.nextUrl.pathname === '/auth/callback';
    if (!isOnCallback) {
      const url = request.nextUrl.clone();
