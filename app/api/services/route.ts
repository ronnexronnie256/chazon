...
export async function GET(req: NextRequest) {
  ...
  const useSmartMatching =
    searchParams.get('smartMatch') === 'true' || (clientLat && clientLng);
  ...
  try {
    ...
    if (useSmartMatching && clientLat && clientLng) {
      ...
      const paginatedOfferings = matchedOfferings.slice(
        (page - 1) * limit,
        page * limit
      );
      ...
      const badgesPromises = stewardIds.map(userId =>
        calculateStewardBadges(userId)
      );
    ...
    return NextResponse.json(
      {
        ...
        error:
          process.env.NODE_ENV === 'development'
            ? `Failed to fetch services: ${errorMessage}`
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user || !user.stewardProfile) {
      return NextResponse.json(
        { error: 'You must be a steward to create services' },
        { status: 403 }
      );
    }
