...
const mapTaskToBooking = (task: any) => {
  ...
  return {
    ...
    scheduledTime: task.scheduledStart.toLocaleTimeString([], {
      hour: '2-digit',
      minute: '2-digit',
    }),
  ...
  try {
    ...
    switch (action) {
      case 'accept':
        ...
        const canAccept =
          isSteward || isAdmin || (!task.stewardId && user.role === 'STEWARD');
        if (!canAccept) {
          return NextResponse.json(
        ...
        if (!isAdmin) {
          ...
          if (!stewardProfile || stewardProfile.status !== 'APPROVED') {
            return NextResponse.json(
              {
                success: false,
                error: 'Steward is not approved to perform this action',
              },
              { status: 403 }
            );
          }
        ...
        if (task.status !== 'OPEN') {
          return NextResponse.json(
            {
              success: false,
              error: `Task cannot be accepted. Current status: ${task.status}`,
            },
            { status: 400 }
          );
        }
        ...
        if (task.isExpired || (task.expiresAt && new Date() > task.expiresAt)) {
          return NextResponse.json(
            {
              success: false,
              error: 'This task has expired and can no longer be accepted',
            },
            { status: 400 }
          );
        }
        ...
        if (!hasPayment) {
          return NextResponse.json(
            {
              success: false,
              error: 'Payment must be completed before accepting task',
            },
            { status: 400 }
          );
        }
        ...
        if (!task.stewardId) {
          ...
          return NextResponse.json(
            {
              success: false,
              error: 'This task is assigned to another steward',
            },
            { status: 403 }
          );
        }
      ...
      case 'confirm':
        ...
        if (task.status !== 'DONE') {
          return NextResponse.json(
            {
              success: false,
              error: `Task cannot be confirmed. Current status: ${task.status}`,
            },
            { status: 400 }
          );
        }
        ...
        if (charges.length !== 1) {
          console.error('Escrow integrity violation: invalid CHARGE count', {
            taskId: task.id,
          ...
          return NextResponse.json(
            {
              success: false,
              error: 'Escrow integrity violation: invalid CHARGE count',
            },
            { status: 400 }
          );
        }
        ...
        if (existingPayout) {
          return NextResponse.json(
            {
              success: false,
              error: 'Payment has already been released to steward',
            },
            { status: 400 }
          );
        }
        ...
        if (milestones.length > 0) {
            ...
            return NextResponse.json(
              {
                success: false,
                error:
                  'All milestones must be completed before final confirmation',
              },
              { status: 400 }
            );
          }
          ...
          const stewardAmount =
            completedTransaction.amount - completedTransaction.platformFee;
        ...
        if (updatedTask?.stewardId) {
          await notifyTaskConfirmed(id, updatedTask.stewardId).catch(
            console.error
          );
      ...
      case 'cancel':
        ...
        if (task.status === 'OPEN' && !isClient && !isAdmin) {
          return NextResponse.json(
            {
              success: false,
              error: 'Only clients can cancel unassigned tasks',
            },
            { status: 403 }
          );
        }
      ...
      case 'no-show':
        ...
        if (!isClient && !isAdmin) {
          return NextResponse.json(
            {
              success: false,
              error: 'Only clients or admins can report no-show',
            },
            { status: 403 }
          );
        }
        ...
        if (task.status !== 'ASSIGNED') {
          return NextResponse.json(
            {
              success: false,
              error: `No-show can only be reported for ASSIGNED tasks. Current status: ${task.status}`,
            },
            { status: 400 }
          );
        }
        ...
        if (!noShowRefundResult.success) {
          return NextResponse.json(
            {
              success: false,
              error: `Failed to process refund: ${noShowRefundResult.error}`,
            },
            { status: 500 }
          );
        }
