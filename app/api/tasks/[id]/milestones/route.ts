  ...
  try {
    ...
    if (task.status !== 'OPEN' && task.status !== 'ASSIGNED') {
      return NextResponse.json(
        {
          success: false,
          error: 'Milestones can only be created for OPEN or ASSIGNED tasks',
        },
        { status: 400 }
      );
    }
    ...
    for (const milestone of milestones) {
      if (!milestone.name || !milestone.amount || milestone.amount <= 0) {
        return NextResponse.json(
          {
            success: false,
            error: 'Each milestone must have a name and positive amount',
          },
          { status: 400 }
        );
      }
    ...
    if (totalAmount > task.agreedPrice) {
      return NextResponse.json(
        {
          success: false,
          error: 'Total milestone amount cannot exceed task price',
        },
        { status: 400 }
      );
    }
