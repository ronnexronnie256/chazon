  ...
  try {
    ...
    if (dayOfWeek !== undefined || startTime || endTime) {
      const finalDayOfWeek =
        dayOfWeek !== undefined ? parseInt(dayOfWeek) : existingSlot.dayOfWeek;
      const finalStartTime = startTime || existingSlot.startTime;
      const finalEndTime = endTime || existingSlot.endTime;
      const finalIsRecurring =
        isRecurring !== undefined ? isRecurring : existingSlot.isRecurring;
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to update availability slot',
      },
      { status: 500 }
    );
  }
